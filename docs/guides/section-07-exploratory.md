# Section 7 — Exploratory Testing with Antigravity (Step-by-Step)

**What you'll produce:** findings from an AI-assisted exploratory testing session — a list
of bugs and oddities Antigravity surfaces by driving the app, which you'll turn into bug
reports in Section 8.

**Process step:** Test execution · **Tools:** Antigravity (session co-pilot) · Claude (documentation)

---

## Before you start
- Antigravity installed and signed in, with the repo folder open
...
could go wrong). Antigravity can both suggest charters and run the session itself.

## Step 1 — Run an autonomous exploratory session (Clip 1/3)
In Antigravity, use **Prompt 40** (point it at the app folder):
...
Antigravity opens the app, clicks through it, and reports findings — each as a title + one
...
Have Antigravity export or copy out its session findings. Save them (e.g.
...
## Common mistakes
- **Accepting every Antigravity finding as a bug** → some are intended behaviour. Confirm each yourself (Step 2).
- **Not saving the session notes** → you'll have nothing to feed Section 8. Export them.
- **Pointing Antigravity at the wrong folder** → it explores the wrong app. Use `techshop/broken-app`.

## Done when
- [ ] Antigravity ran a session on `techshop/broken-app` and reported findings
